            ...
            <div class="col-lg-6 align-self-center pad-0 form-section">
                <div class="form-inner">
                    <a href="<?=$baseurl;?>" class="">
                     ...
                     <div class="mt-2"><h3>We have sent a verification code to<br>+91 <?=$_SESSION['phone'];?></h3></div>
...
<?php
                        ...
                        while ($existingUser) {
                            ...
                            if(($check)){
                                ...
                                if(is_array($data) && count($data)<1){
                                    ...
                                    $createUser = "insert into users (status,created_at,phone,userId,profile_pic,city) values(:status,:created_at,:phone,:userId,:profile_pic,:city)";
                                    $stm = $con->prepare($createUser);
                                    $stm->execute(['status'=>1,'created_at'=>$date,'phone'=>$phone,'userId'=>$userId,'profile_pic'=>'https://landsbazzar.com/img/user.png','city'=>'Dharwad']);
                                    ...
                                    $_SESSION['login']=true;
                                    setcookie('userLoggedIn', true, time() + (30 * 24 * 60 * 60), '/');
                                    setcookie('userId', $data[0]->userId, time() + (30 * 24 * 60 * 60), '/');
                                    echo "<script>window.location='home'</script>";
                    ...
                    <form action=""  method="POST" id="login-form">
                        ...
                        <div class="form-group clearfix">
                            <button type="submit" id="login-btn" class="btn-md btn-theme w-100">Continue</button>
                        </div>
