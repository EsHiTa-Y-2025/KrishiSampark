...
require 'private/sidebar.php';
  ...
  $sql = 'SELECT listings.*,states.state_name,cities.city_name,users.is_verified,users.is_plus,users.is_trusted,categories.cat_name FROM listings 
            inner join states on states.id = listings.state 
            inner join cities on cities.id=listings.city 
            inner join categories on categories.id=listings.category 
            inner join users on users.userId=listings.posted_by
            where  is_deleted=:is_deleted ORDER BY listings.id DESC';
    $stm = $con->prepare($sql);
    $stm->execute(['is_deleted'=>0]);
