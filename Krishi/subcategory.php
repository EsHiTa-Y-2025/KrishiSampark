...
require 'private/sidebar.php';
    ...
    if(isset($_GET['cat_id'])){
        $subCat = get_sub_category_by_cat($con,$_GET['cat_id']);
    }else{
        $subCat=get_sub_category($con,'DESC');    
    }
  ...
  if (in_array($ext, $allowed)) {
    ...
    $insertfile='NA';
        ...
        if($error == ''){
            ...
            $slug = preg_replace('/[^a-z0-9-]+/', '-', trim(strtolower($cat_name.'-'.$parent_id)));
            ...
            $addpackages = "INSERT INTO `categories` (`cat_name`,`cat_icon`,`slug`,`parent_id`,`cat_name_marathi`,`cat_name_kannad`) VALUES (:cat_name,:cat_icon,:slug,:parent_id,:cat_name_marathi,:cat_name_kannad)";
                  ...
                  <tr>
                    ...
                    <td>
                      <a href="edit-category?id=<?=$p->subcategory_id;?>" class="badge badge-primary">Edit</a>
                       <a href="more_categories?id=<?=$p->subcategory_id;?>" class="badge badge-warning">Add More Categories</a> 
                                              <a href="subcategory-listings?sub_cat_id=<?=$p->subcategory_id;?>" class="badge badge-success">View Listings</a> 
